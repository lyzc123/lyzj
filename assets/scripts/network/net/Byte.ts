export default class Byte {
    ...
    public static _sysEndian = null;

    private _xd_: boolean = true;       //是否为小端数据
    ...
    public static getSystemEndian() {
        if (!Byte._sysEndian) {
            ...
            Byte._sysEndian = (new Int16Array(buffer))[0] === 256 ?
    ...
    *<p>如果要设置的长度大于当前已分配的内存空间的字节长度，则重新分配内存空间，大小为以下两者较大者：要设置的长度、当前已分配的长度的2倍，并将原有数据拷贝到新的内存空间中；
        如果要设置的长度小于当前已分配的内存空间的字节长度，也会重新分配内存空间，大小为要设置的长度，并将原有数据从头截断为要设置的长度存入新的内存空间中。</p>
    */
    public get length() {
        return this._length;
    }
    public set length(value) {
        if (this._allocated_ < value) this.___resizeBuffer(this._allocated_ = Math.floor(Math.max(value, this._allocated_ * 2)));
    ...
    *@return 指定长度的字符串。
    */
    public getCustomString(length) {
        let len = length;
        let v = '', ulen = 0, c = 0, c2 = 0, f = String.fromCharCode;
    ...
    */
    public writeArrayBuffer(arraybuffer, offset: number, length: number) {
        let len = length;
        let off = offset;

        if (off < 0 || len < 0) throw 'writeArrayBuffer error - Out of bounds';
        if (len === 0) len = arraybuffer.byteLength - off;
        this.ensureWrite(this._pos_ + len);
        let uint8array = new Uint8Array(arraybuffer);
        this._u8d_.set(uint8array.subarray(off, off + len), this._pos_);
        this._pos_ += len;
    }
    ...
    public writeLong(value: number): void {
        ...
        for (let i = 0; i < 8; i++) {
            let mod = value % 0x100;
            value = (value - mod) / 0x100;
            buf[7 - i] = sign ? mod ^ 0xFF : mod;
        }
        this.writeArrayBuffer(buf, 0, 8);
    }
    ...
    public getLong(): number {
        let bytes = this.getUint8Array(this._pos_, 8);
        let sign = bytes[0] >> 7;
        let sum = 0;
        let digits = 1;
        for (let i = 0; i < 8; i++) {
    ...
    */
    private rUTF(len: number): string {
        let v = '', max = this._pos_ + len, c = 0, c2 = 0, c3 = 0, f = String.fromCharCode;
        let u = this._u8d_; // i = 0;
